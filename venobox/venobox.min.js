...
function closeVbox(){var rtn=option.pre_close_callback(obj,content,blocknum,blocktitle);if(rtn!=undefined&&!rtn){return;}
$('body').removeClass('vbox-open');$('body').off('keydown',escapeHandler);overlay.animate({opacity:0},500,function(){overlay.remove();keyNavigationDisabled=false;obj.focus();option.post_close_callback(obj,content,blocknum,blocktitle);});}
var closeclickclass='.vbox-overlay';if(!option.overlayClose){closeclickclass='.vbox-close';}
$(closeclickclass).click(function(e){if($(e.target).is('.vbox-overlay')||$(e.target).is('.vbox-content')||$(e.target).is('.vbox-close')||$(e.target).is('.vbox-preloader')){closeVbox();}})
$('body').keydown(escapeHandler);return false;});});}});function loadAjax(){$.ajax({url:dest,cache:false}).done(function(msg){content.html('<div class="vbox-inline">'+msg+'</div>');preloadFirst();}).fail(function(){content.html('<div class="vbox-inline"><p>Error retrieving contents, please retry</div>');updateoverlay();})}
function loadIframe(){content.html('<div class="venoframe-container"><iframe class="venoframe" src="'+dest+'"></iframe></div>');updateoverlay();}
function loadVid(autoplay,host){var player,videoid;var stringAutoplay=autoplay?"?rel=0&autoplay=1":"?rel=0";if(host=='vimeo'){player='https://player.vimeo.com/video/';var pezzi=dest.split('/');videoid=pezzi[pezzi.length-1];}else{player='https://www.youtube.com/embed/';videoid=YouTubeGetID(dest);}
content.html('<iframe class="venoframe" webkitallowfullscreen mozallowfullscreen allowfullscreen frameborder="0" src="'+player+videoid+stringAutoplay+'"></iframe>');updateoverlay();}
function YouTubeGetID(url){var ID='';url=url.replace(/(>|<)/gi,'').split(/(vi\/|v=|\/v\/|youtu\.be\/|\/embed\/)/);if(url[2]!==undefined){ID=url[2].split(/[^0-9a-z_\-]/i);ID=ID[0];}
else{ID=url;}
return ID;}
function loadInline(){content.html('<div class="vbox-inline">'+$(dest).html()+'</div>');updateoverlay();}
function preloadFirst(){images=$('.vbox-content').find('img');if(images.length){images.each(function(){$(this).one('load',function(){updateoverlay();});});}else{updateoverlay();}}
function updateoverlay(){blocktitle.html(title);content.find(">:first-child").addClass('figlio');$('.figlio').css('width',framewidth).css('height',frameheight).css('padding',border).css('background',bgcolor);updateol(sonH,finH);content.animate({'opacity':'1'},'slow',post_open_callback(content,blocknum,blocktitle))}
function updateol(){sonH=content.outerHeight();finH=$(window).height();if(sonH+80<finH){margine=(finH-sonH)/2-10;content.css('margin-top',margine);content.css('margin-bottom',margine);}else{content.css('margin-top','40px');content.css('margin-bottom','40px');}
